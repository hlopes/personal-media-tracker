...
1. **HTTP API + Qute pages** — single seam covering all user-visible behavior:
   - `POST /api/me/library {externalId, mediaType, status, rating}` → creates `Library Entry` with `watchlist` (rating forbidden) or `COMPLETED` (rating 1–5 required)
   - `PATCH /api/me/library/{id} {status, rating}` → updates `Library Entry` (transition `watchlist ↔ COMPLETED`, edit `Rating`)
   - `GET /api/me/library?status=COMPLETED|watchlist&page=&size=` → paginated `{entries:[{id,status,rating,mediaItem}], page,size,total}` sorted `createdAt desc`
   - `DELETE /api/me/library/{id}` → `204` hard-delete
   - Qute: `GET /watched` (Watched list), `GET /media/{mediaType}/{externalId}` (detail with Watched/watchlist state), `GET /watchlist`, `GET /app` + `base.html` nav (`watchlist | Watched`)
...
A `User` who has completed `Authentication` and `Email Verification` can maintain a `watchlist` of `MediaItem`s to see later, but cannot record works they have already consumed. They have no way to register a `MediaItem` as watched with a personal assessment, no dedicated `Watched` view to browse rated items, and no way to remove a watched entry. This breaks the promised lifecycle (`watchlist → in progress → completed`) defined in `CONTEXT.md`, leaves the `Status.COMPLETED` enum unused, and forces users to remember ratings outside the tracker.

## Solution

Extend the `Library Entry` lifecycle to use `Status.COMPLETED` for watched works, always paired with a `Rating` 1–5. Provide two entry points: (a) directly mark any `MediaItem` as watched with a Rating from its detail page, and (b) transition an existing `watchlist` entry to `COMPLETED` by choosing a Rating. Expose a direct navigation link to the `Watched` filtered view (`status=COMPLETED`) where each `Library Entry` shows its `Rating` as stars, allows inline Rating edits (1★–5★) and hard-delete via `DELETE`. Reuse the existing `Catalog` lazy-cache and `watchlist` hard-delete semantics, keep all `Catalog` credentials server-side, and keep the UI in Qute + Tailwind CDN with the same visual language as `watchlist`.

## User Stories

1. As an authenticated `User`, I want to mark a `MediaItem` as `Watched` directly from its detail page with a 5-star `Rating` (1–5), so that I can record works I have already consumed without first adding to `watchlist`.
...
5. As an authenticated `User`, I want to add a `MediaItem` as `Watched` even if it was never in my `watchlist`, so that I can capture history without prior planning.
6. As an authenticated `User`, I want to convert an existing `watchlist` `Library Entry` to `Watched` by selecting a `Rating` (`PATCH` with `status=COMPLETED, rating=4`), so that I do not create duplicates.
...
8. As an authenticated `User`, I want to revert a `Watched` entry back to `watchlist` (`PATCH {status=watchlist}`) and have its `Rating` cleared, so that I can fix a mistaken mark.
9. As an authenticated `User`, I want a direct navigation link to my `Watched` list next to `watchlist` in the header when `currentUser` is present, so that I can find it without searching.
...
15. As an authenticated `User`, I want hard-delete to allow re-adding the same `MediaItem` later as `watchlist` or `COMPLETED`, so that curation is reversible.
...
18. As an authenticated `User`, I want the detail page to show “Already in watchlist” + “Mark as Watched” 1–5 picker when `alreadyInwatchlist`, so that I can transition without duplicate POST.
19. As an authenticated `User`, I want the detail page to show both “Add to watchlist” and “Or mark as Watched” 1–5 picker when no `Library Entry` exists, so that I have both entry points.
20. As an authenticated `User`, I want duplicate `POST` for the same `(externalId, mediaType)` to be rejected with `409` (`already in watchlist` or `already in library` for `COMPLETED`), regardless of target `Status`, so that I do not duplicate `Library Entries` (`UNIQUE(user_id, media_item_id)`).
...
24. As an authenticated `User`, I want `watchlist` entries to never have a `Rating` (`POST watchlist` with `rating` → `400 rating not allowed for watchlist`, `PATCH watchlist` with `rating` → `400`), so that the domain stays consistent.
...
26. As an authenticated `User`, I want `Watched` pagination (`page & size & total`, `size` clamped 1–100) to work like `watchlist` (e.g., 3 entries with `size=2` → `page0:2, page1:1`), so that large libraries are navigable.
27. As an authenticated `User`, I want the header to show `watchlist` and `Watched` links only when `currentUser != null`, otherwise `Login`/`Register`, so that the anonymous view is clean.
...
29. As a system, I want existing `watchlist` rows (pre-migration) to remain valid with `rating = NULL` after the new migration, so that the upgrade is seamless.
...
- **Domain vocabulary:** Strictly use `CONTEXT.md` language — `MediaItem` (`MediaType` `MOVIE|TV_SERIES`), `Library Entry` (`User ↔ MediaItem` with `Status` and `Rating`), `Status` (`watchlist` vs `COMPLETED`; `IN_PROGRESS/DROPPED/ON_HOLD` reserved), `watchlist` (`status=watchlist` view; `Backlog` alias), `Watched` (`status=COMPLETED` view; `Completed` is domain term, `Watched` is UI label), `Rating` (1–5 integer; avoid `vote/score`; `stars` is presentation), `Catalog` (TMDB, external, never in browser).
- **ADR respect:**
  ...
  - ADR 0002 (Qute + Tailwind CDN): Keep Qute SSR with Tailwind CDN (`rounded-sm`/`border`/`shadow-sm`, `zinc` palette, `Inter`/`JetBrains Mono`), no `web-bundler`, no DaisyUI. `Watched` cards reuse `watchlist` card styling; stars in `amber` (`amber-100/300/500`) for distinction.
  ...
  - ADR 0004 (watchlist as filtered Library Entry with lazy-cached MediaItem): Keep `MediaItem` lazy-cached on first detail; `Watched` reuses same `UNIQUE(user_id, media_item_id)` — a `MediaItem` can only have one `Library Entry` per `User`, so `watchlist → Watched` is a `PATCH` transition, not a new row. Hard-delete on `DELETE` for both views; no soft-delete yet.
- **Schema:** New migration `V4__add_rating_to_library_entries.sql` — `rating SMALLINT CHECK (rating IS NULL OR rating BETWEEN 1 AND 5)`, `CHECK (status <> 'COMPLETED' OR rating IS NOT NULL)`, `CHECK (status <> 'watchlist' OR rating IS NULL)`, index `(user_id, status, rating)`. Validates that `Completed` must carry `Rating`, `watchlist` must not, and allows future `Status` values to be nullable. Existing `watchlist` rows stay `NULL`.
- **API contracts:**
  - `POST /api/me/library` (auth, `Content-Type: application/json`, body `{externalId: Long! , mediaType: "movie"|"tv"!" , status?: "watchlist"|"COMPLETED" , rating?: 1..5 }`) → `201 {id, status, mediaItem:{id,externalId,mediaType,title,synopsis,posterPath,backdropPath,releaseDate}, createdAt, rating}` or `400` for missing/invalid `rating` per status, `409` if `(userId, mediaItemId)` exists (`already in watchlist` for `watchlist`, `already in library` for `COMPLETED`), `404` if `MediaItem` not found and Catalog cannot lazy-create, `401` if unauth.
  - `PATCH /api/me/library/{id}` (auth, body `{status?, rating?}` at least one required) → `200` updated entry (same shape as `POST`), `400` for `status or rating must be provided`, invalid `rating`, `rating required for COMPLETED`, `rating not allowed for watchlist`, `404` if not owned, `401` if unauth.
  - `GET /api/me/library?status=watchlist|COMPLETED&page=0&size=20` (auth, defaults `watchlist`, `page>=0`, `size` clamped 1–100) → `200 {entries:[...], page,size,total}` sorted `createdAt desc`.
  - `DELETE /api/me/library/{id}` (auth) → `204` hard-delete, `404` if not owned, `401` if unauth.
  - Qute: `GET /watched` renders `watched.html` (auth-gated, `302 /login?error=` if unauth, otherwise `entries, total, currentUser`); `GET /media/{type}/{id}` renders `catalog/detail.html` with `mediaItem, cast, director, posterUrl/backdropUrl, alreadyInwatchlist/alreadyInWatched, currentStatus/currentRating/currentEntryId, currentUser` (fallback to cached `MediaItemDto` with empty `cast/director` when TMDB fails but `MediaItem` is cached; `404` if not found).
...
- **Interactions:**
  - `base.html` header shows `watchlist` + `Watched` links when `currentUser != null`.
...
- **Cases (via HTTP/Qute seam, incremental slices — each slice adds tests and is run before next slice, also checked in browser at `http://localhost:8080` in `quarkus:dev`):**
  - **Slice 1 — API `COMPLETED`+`Rating`:** `401` if unauth for `POST`/`PATCH`/`GET`/`DELETE`; `POST COMPLETED` with `rating 5` → `201` with `rating 5`; `POST COMPLETED` missing `rating` → `400 rating required`; `rating 0`/`6` → `400 between 1 and 5`; `POST watchlist` with `rating` → `400 not allowed`; duplicate `POST` same `(user, mediaItem)` → `409`; `GET ?status=COMPLETED` contains `rating`, `GET ?status=watchlist` does not; `GET /watched` `200` contains `★★★★★` and `href="/media/movie/{id}"`.
  - **Slice 2 — Transitions & edits:** `PATCH {rating:5}` on `COMPLETED` → `200` persists; `PATCH {rating:0}` or `{}` → `400`; `POST watchlist` then `PATCH {status:COMPLETED, rating:4}` → moves from `watchlist` to `Watched` (watchlist size 0, Watched size 1, `GET /watched` shows `★★★★☆`); `PATCH {status:watchlist}` on `COMPLETED` clears `rating` to `null` and moves back.
  - **Slice 3 — Removal, isolation, pagination, detail & nav:** `DELETE` → `204` removes from `GET ?status=COMPLETED` and `GET /watched` (no `★★☆☆☆`); second `DELETE` → `404`; second `User` sees empty `Watched` and `404` on deleting first `User`'s entry; pagination `page=0,size=2` with 3 entries → `total 3, entries 2` and `page=1,size=2` → `1`; `GET /media/movie/{id}` when `COMPLETED` → `200` contains `Watched` + `★★★★☆` + `Update rating` + `Remove from Watched`; when `watchlist` → `Already in watchlist` + `Mark as Watched`; when none → `Add to watchlist` + `Or mark as Watched`; `GET /app` with `Bearer` contains `href="/watchlist"` and `href="/watched"` + `>Watched<`, `GET /` without auth does not.
...
- Video Game `MediaItem`s (`MediaType.GAME`), `Status` values beyond `watchlist` and `COMPLETED` (`IN_PROGRESS`, `DROPPED`, `ON_HOLD`) and their transitions, half-star `Rating`s (e.g., 4.5), `Rating` history/auditing, and `Library Entry` soft-delete.
- Persisting `cast`/`director` in DB, progress percentage, personal notes/comments on entries, or TMDB `vote_average` import.
- Offline-first, bulk import/export of `Watched`/`watchlist`, or CSV/JSON sync.
...
- **Rating presentation:** Stars are presentation only — domain term is `Rating` (1–5). Watched cards and detail use `amber` (`bg-amber-100 border-amber-300 text-amber-500/600`) to distinguish from `watchlist` zinc styling, while still using `rounded-sm`/`border-zinc-200`/`shadow-sm` per ADR 0002.
...
- **Browser verification:** Each slice is verified incrementally with `./mvnw quarkus:dev` at `http://localhost:8080/app` → search → detail → 1–5★ Watched, `GET /watched` stars, inline edit, Remove, and `GET /watchlist` unchanged. Tests mirror this with `GET /watched`/`GET /media/*` HTML assertions (`containsString`).

...
1. **HTTP API + Qute pages** — single seam covering all user-visible behavior:
   ...
   - `POST /api/me/library {externalId, mediaType}` → `watchlist` `Library Entry`; `GET /api/me/library?status=watchlist&page=&size=`; `DELETE /api/me/library/{libraryEntryId}`
   - Qute: `GET /app` (search box), `GET /media/{type}/{id}`, `GET /watchlist` (or `/backlog` alias) + `base.html` nav
...
8. As an authenticated `User`, I want a single “Add to watchlist” button on the detail page, so that I can add the `MediaItem` to my `watchlist` in one click.
9. As an authenticated `User`, I want the “Add to watchlist” button to be disabled / show “Already in watchlist” if my `Library Entry` for that `MediaItem` already exists, so that I don’t create duplicates.
...
11. As an authenticated `User`, I want adding a duplicate `MediaItem` to be idempotent and return `409 {error: "already in watchlist"}`, so that the API is safe to retry.
12. As an authenticated `User`, I want unauthenticated access to `/api/catalog/*`, `/api/me/library`, `/media/*`, `/watchlist` to redirect/`401` to login, so that the `Catalog` cannot be abused.
13. As an authenticated `User`, I want my `watchlist` isolated per `User` (I only see my `Library Entry`s), so that my data is private.
14. As an authenticated `User`, I want a link to my `watchlist` in the account menu/nav (`base.html` when `currentUser != null`), so that I can find it.
15. As an authenticated `User`, I want my `watchlist` page (`GET /watchlist` and `GET /api/me/library?status=watchlist`) to list all my `Library Entry`s as cards (poster, title, `MediaType`, release year, added date), sorted newest first, paginated (20/page), so that I can browse.
16. As an authenticated `User`, I want each `watchlist` entry to have a “Remove” action that calls `DELETE /api/me/library/{libraryEntryId}` and returns `204`, so that I can curate the list.
...
18. As an authenticated `User`, I want `watchlist` removal to hard-delete the `Library Entry` (phase 2) — not merely hide it — so that I can re-add the same `MediaItem` later.
19. As an authenticated `User`, I want `watchlist` entries to show a link back to their detail page, so that I can re-inspect before removing.
...
- **ADR 0004 — Lazy-cached MediaItem + hard-delete watchlist:** Search does not persist; detail `GET /api/catalog/{type}/{id}` fetches `/{movie|tv}/{id}` + `/{movie|tv}/{id}/credits` in parallel, then `findOrCreateMediaItem(externalId, mediaType)` snapshots title/synopsis/poster/backdrop/releaseDate. `POST /api/me/library {externalId, mediaType}` reuses that item and creates `Library Entry(status=watchlist)`. `GET /api/me/library` filters by `status`; `DELETE` hard-deletes (no `DROPPED` yet). Cast/director are returned with detail but not persisted beyond optional 24h cache. Rejected “persist every search result” (write amplification) and “store only externalId” (slow backlog reads).
- **API contracts:**
...
- **Interactions:** `app.html` header search input (vanilla JS, 300ms debounce, `fetch` with cookie/Bearer, renders dropdown `rounded-sm`/`border-zinc-200`/`shadow-sm`, zinc palette, Inter/JetBrains Mono, keyboard nav). Dropdown select → `location=/media/{type}/{id}`. Detail Qute embeds poster (`imageBaseUrl/w500`), backdrop (`w1280`), add button (`POST`), watchlist cards reuse same styling. Nav in `base.html` shows `watchlist` link when `currentUser != null`.
...
- **Cases (via API seam):**
  ...
  - Library add: 401 if unauth, `201` creates `Library Entry`, duplicate `POST` → `409`, invalid `mediaType` → `400`, entry appears in `GET /api/me/library?status=watchlist`, not visible to second `User`.
  ...
  - Qute: `GET /app` contains search box when authed, `GET /media/movie/{id}` renders title/poster/director/cast + add button, `GET /watchlist` renders cards + remove buttons, unauth `GET /watchlist` → `302 /login`.
...
- `Status` transitions beyond `watchlist` (`IN_PROGRESS`, `COMPLETED`, `DROPPED`, `ON_HOLD`), ratings, progress, notes, and `Library Entry` soft-delete/auditing.
- Persisting `cast`/`director` in DB, user ratings, or personal notes on entries.
- Offline-first or bulk import/export of `watchlist`.
